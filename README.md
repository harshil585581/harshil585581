# Hi there! I'm Harshil S 👋 

### 🚀 Full Stack Developer & Smart Automation Specialist
Based in Bengaluru, India. I build high-performance web applications, cross-platform mobile apps, and intelligent workflow automation systems to transform complex operations into seamless digital experiences. 

---

## 🌐 Quick Links
* **Portfolio Website:** [harshils-dev.netlify.app](https://harshils-dev.netlify.app)
* **LinkedIn:** [linkedin.com/in/harshil-s-773233259](https://www.linkedin.com/in/harshil-s-773233259/)
* **Email:** [harshil58558@gmail.com](mailto:harshil58558@gmail.com)

---

## 🛠️ Tech Stack & Expertise

| Category | Tools & Technologies |
| :--- | :--- |
| **Frontend & Mobile** | ![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB) ![Next.js](https://img.shields.io/badge/Next.js-black?style=flat-square&logo=next.js&logoColor=white) ![React Native](https://img.shields.io/badge/React_Native-20232A?style=flat-square&logo=react&logoColor=61DAFB) ![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white) ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black) |
| **Backend & Cloud** | ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![Node.js](https://img.shields.io/badge/Node.js-43853D?style=flat-square&logo=node.js&logoColor=white) ![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white) ![MySQL](https://img.shields.io/badge/MySQL-00758F?style=flat-square&logo=mysql&logoColor=white) ![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat-square&logo=postgresql&logoColor=white) |
| **Workflow Automation**| ![n8n](https://img.shields.io/badge/n8n-FF6C37?style=flat-square&logo=n8n&logoColor=white) ![Python Automation](https://img.shields.io/badge/Python_Scripts-3776AB?style=flat-square&logo=python&logoColor=white) |

---

## 💼 Work & Experience
* **Full Stack Developer** @ **Mavixy** (2025 - Present)
  * Engineering scalable web solutions and APIs.
* **Software Developer** @ **Sudeeksha Technologies** (2024 - 2025)
  * Developed robust web and mobile applications with integrated databases.
* **Software Intern** @ **ATS Power Solutions** (2023 - 2024)
  * Built internal automation workflows and assisted with backend scripts.

---

## 🏆 Featured Projects

### 🛠️ [Zip Safari](https://zipsafari.netlify.netlify.app/) — All-in-One Bulk File Processing Engine
* **Overview:** A bulk-processing SaaS utility built to handle massive file conversion, compression, and upscale.
* **Core Tech:** React, Python, FFmpeg.
* **Key Features:** Upload mixed ZIP archives for bulk conversion (PDF, Word, Images); AI image/video upscaling, PDF encryption, and custom watermarking.

### 📦 [Circlify](https://circlify-oms.netlify.app/) — B2B Order Management System
* **Overview:** An enterprise-grade B2B platform digitizing order fulfillment between wholesalers (Admins) and retail clients.
* **Core Tech:** React 19, TypeScript, Supabase.
* **Key Features:** Dual-portal architecture (Supplier Command Center + Client Storefront), live shipment status, and real-time sales reporting.

### 📱 [Bill-Mobile (WowDash)](https://billmanagment-harshil.netlify.app/) — Financial & Expense System
* **Overview:** A cross-platform finance system keeping a Web Dashboard and Mobile App synchronized in real time.
* **Core Tech:** React, React Native, Supabase, AI OCR (Llama 3 / Tesseract).
* **Key Features:** AI receipt scanning to automatically extract transaction details; real-time dashboard analytics (pie/bar charts), and instant cloud sync.

### 🎬 [Varanasi](https://varanasi-movie.netlify.app/) — Cinematic Movie Showcase Page
* **Overview:** Immersive movie promotional website designed with a dark cinematic theme, responsive layout, and smooth animations.
* **Core Tech:** HTML5, CSS3, JavaScript, Netlify.

---

*“Turning vision into reality with creative innovation. Let's build something scalable together.”*
